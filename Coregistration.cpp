    ...
    if(OpenProject(_filename,proinfo,args))
    {
        ...
        else
        {
            *cal_check = true;
            ...
            vector<D2DPOINT> *matched_MPs = (vector<D2DPOINT>*)calloc(sizeof(vector<D2DPOINT>),proinfo->number_of_images);
            vector<D2DPOINT> *matched_MPs_ref = (vector<D2DPOINT>*)calloc(sizeof(vector<D2DPOINT>),proinfo->number_of_images);
            int reference_id = 0;
            for(int level = py_level ; level >= 0 ; level --)
            {
                ...
                SubImages[reference_id] = LoadPyramidImages(proinfo->tmpdir,Subsetfilename[reference_id],data_size_lr[reference_id][level],level);
                ...
                for(int ti = 1 ; ti < proinfo->number_of_images ; ti ++)
                {
                    ...
                    CoregParam_Image(proinfo, ti, level, ImageAdjust_coreg, 15, SubImages[reference_id], data_size_lr[reference_id][level], SubImages[ti], data_size_lr[ti][level], ImageBoundary[reference_id], ImageBoundary[ti], grid_dxy_ref, grid_dxy_tar, Grid_space[level], Boundary[ti], avg_roh, &iter_counts, adjust_std, matched_MPs, matched_MPs_ref);
                
                
                    printf("%d\t%d\t\t%4.2f\t\t%4.2f\t\t%4.2f\t\t%4.2f\t\t%d\t%4.2f\t%d\n",level,ti,ImageAdjust_coreg[ti][0], ImageAdjust_coreg[ti][1],
                           -ImageAdjust_coreg[ti][0]*ortho_dy[ti], ImageAdjust_coreg[ti][1]*ortho_dx[ti],matched_MPs[ti].size(),avg_roh[ti],iter_counts);
                    free(SubImages[ti]);
                }
            ...
            for(int ti = 0 ; ti < proinfo->number_of_images ; ti ++)
            {
                ...
                if(p_GCP)
                {
                    ...
                    for(long count = 0 ; count < matched_MPs[ti].size() ;count++)
                    {
                        long pos_c = (long)((matched_MPs_ref[ti][count].m_X - Boundary[ti][0])/GCP_grid);
                        long pos_r = (long)((Boundary[ti][3] - matched_MPs_ref[ti][count].m_Y)/GCP_grid);
                    ...
                    free(GCP_value);
                    matched_MPs_ref[ti].clear();
                    matched_MPs[ti].clear();
                }
                free(OriImages[ti]);
                free(ImageBoundary[ti]);
            }
            
            free(matched_MPs_ref);
            free(matched_MPs);
...
#pragma omp parallel for schedule(guided)
        ...
        */
        ...
        for(int ti = 1 ; ti < proinfo->number_of_images ; ti ++)
        {
            ...
            D3DPOINT *save_pts = (D3DPOINT*)calloc(sizeof(D3DPOINT),RA_iter_counts[ti]);
...
#pragma omp parallel for reduction(+:sum_diff,diff_count) schedule(guided)
            ...
            double* save_Z = (double*)calloc(sizeof(double),diff_count);
            double sum_var = 0;
            ...
            double MED_z = quickselect(save_Z, diff_count, (int)(diff_count/2.0));
...
#pragma omp parallel for reduction(+:sum_var_med) schedule(guided)
            ...
            */
            double* save_dz = (double*)calloc(sizeof(double),tar_data_size.width*tar_data_size.height);
            double* save_dz_med = (double*)calloc(sizeof(double),tar_data_size.width*tar_data_size.height);
        ...
        if(Maketmpfolders(proinfo))
        {
            ...
            for(int ti = 1; ti < proinfo->number_of_images ; ti++)
            {
                ...
                for(int level = py_level ; level >= 0 ; level --)
                {
                    ...
                    MPs_2D = CoregParam_Image_MPs_stereo(proinfo, level,py_level, ImageAdjust_coreg,
...
#pragma omp parallel for reduction(+:sum_var_med) schedule(guided)
                    ...
                    if(!check_cps)
                    {
                        ...
                        {
                            save_dz = (double*)calloc(sizeof(double),tar_data_size.width*tar_data_size.height);
                            save_dz_med = (double*)calloc(sizeof(double),tar_data_size.width*tar_data_size.height);
...
D3DPOINT FindNormal(D3DPOINT *normal_ori, float* dem, D3DPOINT Pos,D3DPOINT Mean, D3DPOINT Scale, double* Boundary, Conformalparam X, CSize tinsize, double Gridspace, double *roh_array, double *Z, bool check_tar)
{
    ...
    double geo_X = Pos.m_X*Scale.m_X + Mean.m_X;
    double geo_Y = Pos.m_Y*Scale.m_Y + Mean.m_Y;
    //printf("findnormal %f\t%f\n",geo_X,geo_Y);
    double col_float = (geo_X- Boundary[0])/Gridspace;
    double row_float = (Boundary[3]-geo_Y)/Gridspace;
       ...
       index51 >= 0 && index51 < tinsize.width*tinsize.height && row + 1 < tinsize.height && row - 1 >= 0 && col + 1 < tinsize.width && col -1 >= 0)
    {
           ...
           dem[index31] > -100 && dem[index32] > -100 && dem[index33] > -100 && dem[index41] > -100 && dem[index51] > -100)
        {
            ...
            double mag = SQRT(n);// sqrt(n.m_X*n.m_X + n.m_Y*n.m_Y + n.m_Z*n.m_Z);
            n.m_X /= mag;
            n.m_Y /= mag;
            n.m_Z /= mag;
            ...
            *Z = - (Pos.m_X*n.m_X + Pos.m_Y*n.m_Y + d)/n.m_Z;
            ...
            mag = SQRT(n);// sqrt(n.m_X*n.m_X + n.m_Y*n.m_Y + n.m_Z*n.m_Z);
            n.m_X /= mag;
            n.m_Y /= mag;
            n.m_Z /= mag;
...
D3DPOINT SurfaceDistance_ori(D3DPOINT tar_normal_ori, float* ref_dem, D3DPOINT tar_normal, D3DPOINT tar_pts, double *tin_boundary,CSize tinsize, double Gridspace, Conformalparam param, D3DPOINT Mean, D3DPOINT Scale, float p_ref_z)
{
    ...
    D3DPOINT tar_normal_denor;
    tar_normal_denor.m_X    = tar_normal_ori.m_X;
    tar_normal_denor.m_Y    = tar_normal_ori.m_Y;
    tar_normal_denor.m_Z    = tar_normal_ori.m_Z;
    
    double mag              = SQRT(tar_normal_denor);// sqrt(tar_normal_denor.m_X*tar_normal_denor.m_X + tar_normal_denor.m_Y*tar_normal_denor.m_Y + tar_normal_denor.m_Z*tar_normal_denor.m_Z);
    ...
    double vector_slope_x      = atan2(fabs(tar_normal_denor.m_Z),fabs(tar_normal_denor.m_X));
    double vector_slope_y      = atan2(fabs(tar_normal_denor.m_Z),fabs(tar_normal_denor.m_Y));
        
    //set up for start points
    D3DPOINT line_node_pt = Denormalize_coord(tar_pts,Mean,Scale);
    D3DPOINT pre_pt_on_surface   = line_node_pt;
    pre_pt_on_surface.m_Z = p_ref_z*Scale.m_Z + Mean.m_Z;
    
    D3DPOINT after_pt_on_surface = pre_pt_on_surface;
    int max_iter            = 10;
    int count = 1;
    bool check_stop = false;
    while(count < max_iter && !check_stop)
    {
        ...
        double dx                        = pre_pt_on_surface.m_X - after_pt_on_surface.m_X;
        double dy                        = pre_pt_on_surface.m_Y - after_pt_on_surface.m_Y;
        double dz                        = pre_pt_on_surface.m_Z - after_pt_on_surface.m_Z;
        ...
        {
            double pre_angle_x               = atan2(fabs(dz),fabs(dx));
            double pre_angle_y               = atan2(fabs(dz),fabs(dy));
...
void SlopeAspect(D3DPOINT normal, D3DPOINT scale, double *slope, double *aspect)
{
    ...
    double denominator = SQRT(normal);// sqrt(normal.m_X*normal.m_X + normal.m_Y*normal.m_Y + normal.m_Z*normal.m_Z);
    double value = normal.m_Z/denominator;
...
D3DPOINT* CreateImagePyramid_DEM(float* _input, double grid_size, double *boundary, double* overlapped_br, uint8 pyramid_level, CSize _img_size, int _filter_size, double _sigma, float* result_img, long *tin_point_num, bool check_pts)
{
    ...
    *tin_point_num = 0;
    ...
    if(pyramid_level > 0)
    {
        ...
        if(check_pts)
        {
            coord = (D3DPOINT*)calloc(sizeof(D3DPOINT),(long)result_size.height*(long)result_size.width);
            if (coord == NULL)
            {
    ...
    else
    {
        ...
        if(check_pts)
        {
            coord = (D3DPOINT*)calloc(sizeof(D3DPOINT),data_length);
                ...
                for(i = 0; i<total_grid_counts ; i++)
                {
                       ...
                       && Right_Imagecoord.m_Y < RImagesize.height - Half_template_size*b_factor - 10    && Right_Imagecoord.m_X < RImagesize.width - Half_template_size*b_factor - 10)
                    {
                        ...
                        if( (index_l > 0 && index_l < LImagesize.height*LImagesize.width) && (index_r > 0 && index_r < RImagesize.height*RImagesize.width) && Images[reference_id][index_l] > 0 && Images[ti][index_r] > 0 )
                        {
                            ...
                            //if(postNCC_ortho(Pyramid_step, ori_diff, Left_Y,  Left_X, Right_Y, Right_X, subA,TsubA,InverseSubA,Template_size,_flag,1,LImagesize,RImagesize,Images[reference_id],Images[ti],&t_sum_weight_X,&t_sum_weight_Y,&t_sum_max_roh))
                            {
...
D2DPOINT* CoregParam_Image_MPs_stereo(ProInfo *proinfo,uint8 Pyramid_step, uint8 total_level, double *ImageAdjust,
                ...
                for(i = 0; i<total_grid_counts ; i++)
                {
                       ...
                       && Right_Imagecoord.m_Y < RImagesize.height - Half_template_size*b_factor - 10    && Right_Imagecoord.m_X < RImagesize.width - Half_template_size*b_factor - 10)
                    {
                        ...
                        if( (index_l > 0 && index_l < LImagesize.height*LImagesize.width) && (index_r > 0 && index_r < RImagesize.height*RImagesize.width) && Images_ref[index_l] > 0 && Images_tar[index_r] > 0 )
                        {
                            ori_diff = 0;
                            F2DPOINT peak_pos;
                            //if(postNCC_ortho(Pyramid_step, ori_diff, Left_Y,  Left_X, Right_Y, Right_X, subA,TsubA,InverseSubA,Template_size,1,LImagesize,RImagesize,Images_ref,Images_tar,&t_sum_weight_X,&t_sum_weight_Y,&t_sum_max_roh,&peak_pos))
                            {
                                flag_MPs[i] = true;
...
{
    double* C = (double*)calloc(sizeof(double),21);
    
    RM R = MakeRotationMatrix(param.omega, param.phi, param.kappa);
    
    double A1 = param.omega*DegToRad;
    double A2 = param.phi*DegToRad;
    double A3 = param.kappa*DegToRad;
    double S = param.scale;
    ...
    {
        float P1 = transformed_coord[count].m_X;
        float P2 = transformed_coord[count].m_Y;
        float P3 = transformed_coord[count].m_Z;
        
        float Gx = tar_normal[count].m_X;///coord_scale.m_X;
        float Gy = tar_normal[count].m_Y;///coord_scale.m_Y;
        float Gz = -tar_normal[count].m_Z;///coord_scale.m_Z;
