...
int main(int argc,char *argv[])
{
	...
	else if(argc == 2)
	{
		if (strcmp("-help",argv[1]) == 0) 
		{
			...
			printf("setsm verion : 2.06062016\n");
...
void SETSMmainfunction(char* _filename, ARGINFO args, char *_LeftImagefilename, char *_save_filepath)
{
	...
	{
		float Boundary[4]	= {0.0};
		float LBoundary[4],RBoundary[4],LminmaxHeight[2],RminmaxHeight[2],ori_minmaxHeight[2];
		float LHinterval[1], RHinterval[1], Hinterval;
		float Image_res[2]	= {0.0};
		float Res[2]		= {0.0};
		float Limageparam[2] = {0.0};
		float Rimageparam[2] = {0.0};
		
		double **LRPCs, **RRPCs, minLat;
        ...
        if(Maketmpfolders(proinfo))
		{
			...
			minLat		= LRPCs[0][3];
			SetTransParam((float)(minLat), &Hemisphere, &param);
			...
			if(SetupParam(proinfo,&NumOfIAparam, &pre_DEM_level, &DEM_level,&proinfo.pre_DEMtif,&check_tile_array ))
			{
				...
				if(proinfo.IsRA)
				{
					...
					if(args.check_seeddem)
					{
                        ...
                        char RAfile[full_size-7];
                        for(i=0;i<full_size-7;i++)
				...
				if(!pFile_DEM)
				{
					...
					mt_grid_size = MergeTiles(proinfo,iter_row_end,t_col_end,420);
					ET = time(0);
					gap = difftime(ET,ST);
				...
				seeddem_size  = Envihdr_reader_seedDEM(hdr_path, &tminX, &tmaxY, &tgrid_size);
						...
						*/
						while(lower_level_match && level >= DEM_level)
						{
							...
							if(proinfo.IsRA)
							{
								...
								GridPT	= SetGrids(&dem_update_flag, flag_start, level, proinfo.resolution, &Size_Grid2D, proinfo.pre_DEMtif,proinfo.priori_DEM_tif, proinfo.DEM_resolution, minmaxHeight,
									&py_resolution, &grid_resolution, subBoundary);
							...
							Grid_wgs = ps2wgs(true,param,Size_Grid2D.width*Size_Grid2D.height,GridPT);
							...
							while(Th_roh >= Th_roh_min)
							{
								...
								*/
								...
								nccresult	= VerticalLineLocus(SubMagImages_L,SubMagImages_R,grid_resolution, Image_res[0],LRPCs,RRPCs,Limagesize,data_size_l[level],SubImages_L,Rimagesize,data_size_r[level],SubImages_R,Template_size,Size_Grid2D,
								...
								count_MPs = SelectMPs(nccresult,Size_Grid2D,GridPT,GridPT3,Th_roh,Th_roh_min,Th_roh_start,Th_roh_next,level,pyramid_step,
									iteration,0,filename_mps_pre,proinfo.pre_DEMtif,proinfo.IsRA,MPP);
								...
								if (check_ortho_cal && proinfo.IsRA != 1)
								{
									...
									count_blunder = DecisionMPs(false,count_MPs,subBoundary,GridPT3,level,grid_resolution,iteration,Size_Grid2D,filename_mps_pre,filename_mps_anchor,proinfo.save_filepath,
																...
																SubMagImages_L,SubMagImages_R,grid_resolution, Image_res[0],LRPCs,RRPCs,
																Limagesize,data_size_l[level],SubImages_L,Rimagesize,data_size_r[level],SubImages_R,Template_size,
																param, Grid_wgs,GridPT,
																NumOfIAparam, t_Rimageparam, Lstartpos, Rstartpos, 
								...
								if(lower_level_match)
								{
									...
									else
									{
										...
										else
										{
											...
											TINgeneration(true,proinfo.save_filepath, level, Size_Grid2D, Image_res[0], grid_resolution, 
														  scaled_ptslists,
										...
										if(check_ortho_cal && proinfo.IsRA != 1)
										{
											...
											count_blunder = DecisionMPs_setheight(true,count_MPs,subBoundary,GridPT3,level,grid_resolution,iteration,Size_Grid2D,filename_mps,proinfo.save_filepath,
																		...
																		param, Grid_wgs,GridPT,
																		NumOfIAparam, t_Rimageparam, Lstartpos, Rstartpos, 
																		proinfo.save_filepath,row,col,ptslists,trilists,count_tri);
										...
										GridPT3		= SetHeightRange(proinfo.pre_DEMtif,minmaxHeight,count_MPs, count_tri, GridPT3,update_flag,&minH_grid,&maxH_grid,blunder_param,ptslists,trilists,proinfo.IsRA,MPP);
			  ...
			  uint8 *RA_row_start, uint8 *RA_row_end, uint8 * RA_row_iter, uint8 *t_col_start, uint8 *t_col_end, uint8 *RA_col_iter, float *subX, float *subY)
{
	float lengthOfX	= Boundary[2] - Boundary[0];
	float lengthOfY = Boundary[3] - Boundary[1];
	uint8 iter_row_start, iter_row_end;
	...
	*RA_row_start	= iter_row_start + 1;
	*RA_row_end		= iter_row_end   - 1;

	if(*RA_row_end < *RA_row_start)
	{
		*RA_row_start	= iter_row_start;
		*RA_row_end		= iter_row_end;
		*RA_row_iter	= 1;
	}
	else
		*RA_row_iter	= 2;
	...
	if(division_X <= 3)
	{
		*RA_row_start	= (uint8)(ceil((float)(division_X/2.0)));
		*RA_row_end		= (uint8)(ceil((float)(division_X/2.0)));
		*t_col_start	= (uint8)(ceil((float)(division_X/2.0)));
		*t_col_end		= (uint8)(ceil((float)(division_X/2.0)));
	}
				   ...
				   float *py_resolution, float *grid_resolution, float *subBoundary)
{
	...
	else
	{
		if(*dem_update_flag)
		{
			Re_Grid_size.width	= Size_Grid2D->width*2;
			Re_Grid_size.height	= Size_Grid2D->height*2;
			Size_Grid2D->width  = Re_Grid_size.width;
			Size_Grid2D->height  = Re_Grid_size.height;
			subBoundary[2]		= subBoundary[0] + (Re_Grid_size.width )*(*py_resolution);
			subBoundary[3]		= subBoundary[1] + (Re_Grid_size.height)*(*py_resolution);
			GridPT 				= SetDEMGrid(subBoundary, *py_resolution, *py_resolution,Size_Grid2D);
		}
		else
		{
				  ...
				  char* priori_DEM_tif,bool pre_DEMtif, float seedDEMsigma, int IsRA,char* metafilename)
{
	...
	if(!flag_start)
	{
		...
		for(i=0;i<total_grid_counts;i++)
		{
			...
			GridPT3[i].Matched_height   = -100.0;
				 ...
				 float *subBoundary, float *minmaxHeight, int *cols, int *rows)
{
	...
	if(GetImageSize(ImageFilename,Imagesize))
	{
		...
		t_pts1			= ps2wgs_3D(true,transparam,8,t_pts);
...
F2DPOINT *SetDEMGrid(float *Boundary, float Grid_x, float Grid_y, CSize *Size_2D)
{
	...
	if(Size_2D->height == 0)
	{
		Size_2D->width	= (int)(ceil((Boundary[2] - Boundary[0])/Grid_x));
		Size_2D->height	= (int)(ceil((Boundary[3] - Boundary[1])/Grid_y));
	}
	
	GridPT	= (F2DPOINT*)malloc(sizeof(F2DPOINT)*Size_2D->height*Size_2D->width);
	#pragma omp parallel for shared(GridPT,Size_2D,Grid_y,Boundary,Grid_x) private(row,col)
	for(row = 0 ; row < Size_2D->height ; row++)
		for(col = 0; col < Size_2D->width ; col++)
		{
...
{
	float minLon, maxLon, minLat, maxLat, minX, maxX, minY, maxY;
	float oriminmaxH[2];
	F2DPOINT *XY,*lonlat;

	minLon			= (float)(-1.2*_rpcs[1][2] + _rpcs[0][2]);
	maxLon			= (float)( 1.2*_rpcs[1][2] + _rpcs[0][2]);
	minLat			= (float)(-1.2*_rpcs[1][3] + _rpcs[0][3]);
	maxLat			= (float)( 1.2*_rpcs[1][3] + _rpcs[0][3]);
	...
	lonlat			= (F2DPOINT*)malloc(sizeof(F2DPOINT)*4);
	lonlat[0].m_X	= minLon;
	lonlat[0].m_Y	= minLat;
	lonlat[1].m_X	= minLon;
	lonlat[1].m_Y	= maxLat;
	...
	lonlat[3].m_X	= maxLon;
	lonlat[3].m_Y	= minLat;
	XY				= wgs2ps(1,_param,4,lonlat);

	minX			= min(XY[3].m_X,min(XY[2].m_X,min(XY[0].m_X,XY[1].m_X)));
	maxX			= max(XY[3].m_X,max(XY[2].m_X,max(XY[0].m_X,XY[1].m_X)));

	minY			= min(XY[3].m_Y,min(XY[2].m_Y,min(XY[0].m_Y,XY[1].m_Y)));
	maxY			= max(XY[3].m_Y,max(XY[2].m_Y,max(XY[0].m_Y,XY[1].m_Y)));
	...
	_imagesize->height	= (int)(ceil((_boundary[3] - _boundary[1])/_res[1]));
	_imagesize->width	= (int)(ceil((_boundary[2] - _boundary[0])/_res[0]));
	...
	if(GetImageSize(LImageFilename,LImagesize) && GetImageSize(RImageFilename,RImagesize))
	{
			...
			GetsubareaImage(transparam,NumofIAparam,RRPCs,RImageParam,RImageFilename,RImagesize,subBoundary,minmaxHeight,Rcols,Rrows) )
		{
			...
			if(rightimage)
			{
				...
				unsigned int data_length = result_size.height*result_size.width;
	...
	*/
	...
	if(m_bOpenMP)
	{
		...
		for(i=0;i<m_NumOfPts;i++)
		{
			...
			chi		= PI/2 - 2*atan(t);
			phi		= chi   + (e2/2 + 5*e4/24 + e6/12 + 13*e8/360)*sin(2*chi)
...
F2DPOINT* GetObjectToImageRPC(double **_rpc, uint8 _numofparam, float *_imageparam, uint16 _numofpts, F3DPOINT *_GP)
{
	...
	if(_numofpts > 1000)
	{
		#pragma omp parallel for shared(_numofpts, _GP, _rpc, _imageparam, _numofparam, IP) private(i)
...
uint16* CreateImagePyramid(uint16* _input, CSize _img_size, int _filter_size, float _sigma)
{
	...
	for(i=-(int)(_filter_size/2);i<(int)(_filter_size/2)+1;i++)
	{
		for(j=-(int)(_filter_size/2);j<(int)(_filter_size/2)+1;j++)
		{
	...
	#pragma omp parallel for shared(_filter_size,sum,GaussianFilter) private(i,j)
	for(i=-(int)(_filter_size/2);i<(int)(_filter_size/2)+1;i++)
	{
		for(j=-(int)(_filter_size/2);j<(int)(_filter_size/2)+1;j++)
		{
	...
	#pragma omp parallel for shared(result_size, _filter_size, _img_size, GaussianFilter, _input, result_img) private(temp, r,c,l,k)
	for(r=0;r<result_size.height;r++)
	{
		for(c=0;c<result_size.width;c++)
		{
			temp = 0;

			for(l=0;l<_filter_size;l++)
			{
...
void MakeSobelMagnitudeImage(CSize _img_size, uint16* _src_image, uint16* _dist_mag_image, /*int16* _gx, int16* _gy,*/ int16* _dir)
{
	...
	#pragma omp parallel for shared(_img_size,/*_gx,_gy,*/_dist_mag_image,hx,hy,_src_image) private(i,j,temp_ptr_X,temp_ptr_Y,temp_ptr)
	for(i=0;i<_img_size.height;i++)
	{
		int16 _gx, _gy;
		for(j=0;j<_img_size.width;j++)
		{
			...
			{
				temp_ptr_X=((hx[0][0]*_src_image[(i-1)*_img_size.width + (j-1)]+hx[0][1]*_src_image[(i-1)*_img_size.width + j]+hx[0][2]*_src_image[(i-1)*_img_size.width + (j+1)]+
				...
				temp_ptr_Y=((hy[0][0]*_src_image[(i-1)*_img_size.width + (j-1)]+hy[0][1]*_src_image[(i-1)*_img_size.width + j]+hy[0][2]*_src_image[(i-1)*_img_size.width + (j+1)]+
				...
				temp_ptr=sqrt(temp_ptr_X*temp_ptr_X + temp_ptr_Y*temp_ptr_Y);
...
{
	long long int rowsize   = imagesize.height;
	long long int colsize   = imagesize.width;
 
    //Procedure
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    int Half_template_size  = (int)(Template_size/2);
 
    long long int main_col, main_row,mask_row,mask_col;
    main_col = colsize;
    main_row = rowsize;

	#pragma omp parallel for shared(main_row,main_col,Half_template_size,Gmag,Gdir,plhs) private(mask_row,mask_col)
    for(mask_row = 0 ; mask_row < main_row; mask_row++)
    {
        for(mask_col = 0 ; mask_col < main_col ; mask_col++)
        {
            double bin[18] = {0.0};
            double sub_bin[4] = {0.0};
            double sub_ratio = 18.0/4.0;
            double bin_angle = 360.0/18.0;
            double max_th = -100;
            long long int th,row,col,sub_th,i;
            
            double angle_sigma = 1.5;
            double gu_weight;

            //calulation of major direction, refering to SIFT descriptor based on gradient
            for(row = -Half_template_size+1 ; row <= Half_template_size-1 ; row++)
            {
                for(col = -Half_template_size+1 ; col <= Half_template_size-1 ; col++)
                {
                    double radius  = sqrt((double)(row*row + col*col));
                    long long int pixel_row       =  mask_row + row;
                    long long int pixel_col       =  mask_col + col;
                    if(radius <= Half_template_size-1 && pixel_row > 0 && pixel_row < main_row-1 && pixel_col > 0 && pixel_col < main_col-1)
                    {
						long long int edge_check = 1;
                        double mag, theta;
						long long int index;
                        gu_weight          = exp(-(double)(row*row + col*col)/(2*angle_sigma*angle_sigma));
                        
                        mag     = Gmag[pixel_row*main_col + pixel_col];
                        theta   = (double)(Gdir[pixel_row*main_col + pixel_col]);
                        if(theta < 0 )
                             theta += 360;
                        index          = (int)(theta/bin_angle);
            ...
            max_th = -100;
            for(i=(int)(sub_th*sub_ratio);i<(int)(sub_th*sub_ratio + sub_ratio);i++)
            {
                if(bin[i] > max_th)
                {
                    max_th = bin[i];
            ...
            plhs[mask_row*main_col + mask_col] = th;
        }
    }
}
					   ...
					   bool pre_DEMtif, char* tmpdir,float *meters_per_pixel, bool IsRA)
{
	...
	*meters_per_pixel = (minmaxHeight[1] - minmaxHeight[0]) / sqrt( pow(temp_p1.m_X - temp_p2.m_X,2.0) + pow(temp_p1.m_Y - temp_p2.m_Y,2.0));	
	...
	nccresult = (NCCresult*)calloc(sizeof(NCCresult),numofpts);
	...
	if( check_ortho )
	{
		#pragma omp parallel for shared(sub_imagesize_w,sub_imagesize_h,all_left_im_cd,all_right_im_cd,LRPCs,RRPCs,NumofIAparam,ImageAdjust,Lstartpos,Rstartpos,Pyramid_step,Grid_wgs,GridPT3) private(iter_count)
...
#pragma omp parallel for shared(height_step,ncc_alpha,ncc_beta,Mag_th,h_divide,sub_imagesize_w,sub_imagesize_h,Size_Grid2D,bin_angle,flag,nccresult,numofpts,LRPCs,RRPCs,NumofIAparam,ImageAdjust,Grid_wgs,Half_template_size,LImagesize_ori,RImagesize_ori,subBoundary,pre_DEMtif,Pyramid_step,GridPT3,Lstartpos,Rstartpos,LeftImage,RightImage,left_ori,right_ori,LImagesize,RImagesize,iteration,minmaxHeight) private(iter_count)
...
float VerticalLineLocus_seeddem(uint16 *MagImages_L,uint16 *MagImages_R,float DEM_resolution, float im_resolution, double** LRPCs, double** RRPCs,
							   ...
							   char* save_filepath, uint8 tile_row, uint8 tile_col, uint8 iteration,uint8 bl_count,float* Boundary, float* minmaxHeight, float seedDEM_sigma)
{
	printf("minmax %f %f\n",minmaxHeight[0],minmaxHeight[1]);
...
#pragma omp parallel for shared(sub_imagesize_w,sub_imagesize_h,all_left_im_cd,all_right_im_cd,LRPCs,RRPCs,NumofIAparam,ImageAdjust,Lstartpos,Rstartpos,Pyramid_step,Grid_wgs,GridPT3) private(iter_count) 
	...
	int count_low = 0;
	//printf("start cal\n");
#pragma omp parallel for shared(nccresult1,sub_imagesize_w,sub_imagesize_h,Size_Grid2D,numofpts,Grid_wgs,Half_template_size,Pyramid_step,GridPT3,LeftImage,RightImage,LImagesize,RImagesize) private(iter_count) reduction(+:count_low, count_total)
	for(iter_count = 0 ; iter_count < Size_Grid2D.height*Size_Grid2D.width ; iter_count++)
	{
		int pts_row = (int)(floor(iter_count/Size_Grid2D.width));
		int pts_col = iter_count % Size_Grid2D.width;
		...
		if(pt_index < Size_Grid2D.width * Size_Grid2D.height && pts_row < Size_Grid2D.height && pts_col < Size_Grid2D.width && pts_row >= 0 && pts_col >= 0)
		{
			...
			if(nccresult < 0.2)
			{
				...
				if(seedDEM_sigma > 10)
				{
					GridPT3[pt_index].minHeight		= (int)(minmaxHeight[0] - 0.5);
					GridPT3[pt_index].maxHeight		= (int)(minmaxHeight[1] + 0.5);
				}
...
#pragma omp parallel for shared(sub_imagesize_w,sub_imagesize_h,all_left_im_cd,all_right_im_cd,LRPCs,RRPCs,NumofIAparam,ImageAdjust,Lstartpos,Rstartpos,Pyramid_step,Grid_wgs,GridPT3) private(iter_count)
	...
	//printf("start cal\n");
#pragma omp parallel for shared(sub_imagesize_w,sub_imagesize_h,Size_Grid2D,nccresult,numofpts,Grid_wgs,Half_template_size,Pyramid_step,GridPT3,LeftImage,RightImage,LImagesize,RImagesize) private(iter_count)
	for(iter_count = 0 ; iter_count < Size_Grid2D.height*Size_Grid2D.width ; iter_count++)
	{
        {
			int pts_row = (int)(floor(iter_count/Size_Grid2D.width));
	...
	*iter_col = interval_col;
	...
	//#pragma omp parallel shared(interval_col,interval_row,savepath,subBoundary,interval_X,interval_Y,interval_buffer,grid_resolution,ptslists) private(iter_count) reduction(+:total_mps_counts,total_tri_counts)
	{
		...
		for(iter_count = 0; iter_count < interval_row*interval_col ; iter_count++)
		{
			...
			//printf("count mps %d\n",count_MPs_nums);
			selected_ptslists = (F3DPOINT*)malloc(sizeof(F3DPOINT)*count_MPs_nums);
...
int DecisionMPs(bool flag_blunder, int count_MPs_input, float* Boundary, UGRID *GridPT3, uint8 Pyramid_step, float grid_resolution, 
	...
	if(count_MPs > 3)
	{
		...
		if(survey)
		{
            ...
            if( !(Pyramid_step == 0 && iteration == 3) )
            {
                ...
                {
                    //TIN split
                    TINgeneration(false,save_filepath, Pyramid_step, Size_Grid2D, im_resolution, DEM_resolution,
                                  scaled_ptslists,
                ...
                while(flag == true && count < max_count && count_tri > 20)
                {
                    ...
                    if(IsRA != 1)
                    {
                        ...
                        VerticalLineLocus_blunder(ortho_ncc, MagImages_L,MagImages_R,DEM_resolution, im_resolution, LRPCs, RRPCs, LImagesize_ori, LImagesize,
                                                              ...
                                                              save_filepath, tile_row, tile_col, iteration,count,Boundary);
                    }
                    //printf("blunder_detection_TIN %d\n",count);
                    blunder_detection_TIN(pre_DEMtif,ortho_ncc,flag_blunder,count,&blunder_dh,filename_mps,ptslists, count_MPs, 
                                          trilists,count_tri, GridPT3, blunder_param, blunder_count,minz_mp,maxz_mp,mt_minmaxheight,IsRA);
                    ...
                    {
                        TINgeneration(!flag,save_filepath, Pyramid_step, Size_Grid2D, im_resolution, DEM_resolution, 
                                      scaled_ptslists,
                                      Boundary, count_MPs, ptslists, &iter_row, &iter_col, &count_tri);
...
bool blunder_detection_TIN(int pre_DEMtif,float* ortho_ncc,bool flag_blunder,uint16 count_bl,float* blunder_dh,char *file_pts, 
						   ...
						   float *minz_mp, float *maxz_mp, float *minmaxHeight, int IsRA)
{
    ...
    if(!(pyramid_step == 0 && iteration == 3))
    {
		...
		float lw_3sigma, up_3sigma;
          ...
          */
			...
			*/ 
          #pragma omp parallel for shared(ortho_ncc_th,pts,tris,savenode,nodecount,num_points,count_height_th,height_th,boundary,gridsize,Gridpts,pyramid_step,iteration) private(index) reduction(+:count_small)
					...
					for (Row=PixelMinXY[1]; Row <= PixelMaxXY[1]; Row++)
					{
						...
						if(!m_bHeight[Index])	
						{
							...
							if (rtn)
							{
								...
								pre_minHeight	= GridPT3[Index].minHeight;
								pre_maxHeight	= GridPT3[Index].maxHeight;
								...
								if(GridPT3[Index].Matched_flag == 4) //extension minHeight
								{
									if(t1 - BufferOfHeight <= GridPT3[Index].minHeight)
										GridPT3[Index].minHeight   = (int)(t1 - BufferOfHeight - 0.5);
								}
								...
								if(GridPT3[Index].Matched_flag == 4)
								{
									if(t2 + BufferOfHeight >= GridPT3[Index].maxHeight)
										GridPT3[Index].maxHeight   = (int)(t2 + BufferOfHeight + 0.5);
								}
								else// if(GridPT3[Index].Matched_flag != 2)
								{
									if(t2 + BufferOfHeight <= GridPT3[Index].maxHeight)
								...
								if(GridPT3[Index].Matched_flag == 2)
								{
									...
									*/
									if(mintemp > GridPT3[Index].minHeight)
										GridPT3[Index].minHeight = (int)(mintemp - 0.5);
									if(maxtemp < GridPT3[Index].maxHeight)
										GridPT3[Index].maxHeight = (int)(maxtemp + 0.5);
								...
								else
								{
									...
									*/
									if(Z <= GridPT3[Index].minHeight)
										GridPT3[Index].minHeight = (int)(Z - BufferOfHeight - 0.5);
									if(Z >= GridPT3[Index].maxHeight)
										GridPT3[Index].maxHeight = (int)(Z + BufferOfHeight + 0.5);
					...
					else
					{
						if(pyramid_step <= 2)
						{
							if(GridPT3[Index].minHeight < Total_Min_Z - BufferOfHeight)
								GridPT3[Index].minHeight   =  (int)(Total_Min_Z - BufferOfHeight - 0.5);
							if(GridPT3[Index].maxHeight > Total_Max_Z + BufferOfHeight)
								GridPT3[Index].maxHeight   =  (int)(Total_Max_Z + BufferOfHeight + 0.5);
						}
...
#pragma omp parallel for shared(TIN_Grid_Size_X,TIN_Grid_Size_Y,GridPT3,result,m_bHeight,Total_Min_Z,Total_Max_Z) private(k,j) 
        for(k=0;k<(int)(TIN_Grid_Size_Y);k++)
        {
            for(j=0;j<(int)(TIN_Grid_Size_X);j++)
            {
				...
				if(m_bHeight[matlab_index] == 0)
					result[matlab_index].Height = -100.0;
			}
        }
    }
					 ...
					 float bin_angle, uint8 total_pyramid, bool Hemisphere, char* save_filepath, char* tmpdir)
{
	...
	Coord			= ps2wgs_3D(true,param,NumofPts,MPs);
					...
					for(col = -Half_template_size; col <= Half_template_size ; col++)
					{
						...
						if(radius <= Half_template_size-1)
						{
								...
								pos_row_left-3 >= 0 && pos_row_left+3 < L_rowsize && pos_col_left-3 >= 0 && pos_col_left+3 < L_colsize)
							{
								...
								if((dx != 0 || dy != 0) && inter_flag == 1)
								{
									left_patch       =  (double)(_leftimage[ (int)(pos_col_left)	+  (int)(pos_row_left)*L_colsize])   *(1-dx)*(1-dy) + 
														(double)(_leftimage[((int)(pos_col_left)+1)	+  (int)(pos_row_left)*L_colsize])   *(dx)  *(1-dy) + 
														(double)(_leftimage[ (int)(pos_col_left)	+ ((int)(pos_row_left)+1)*L_colsize])*(1-dx)*(dy) + 
														(double)(_leftimage[((int)(pos_col_left)+1)	+ ((int)(pos_row_left)+1)*L_colsize])*(dx)  *(dy);
								}
								else
									left_patch		 =  (double)(_leftimage[(int)(pos_col_left)		+  (int)(pos_row_left)*L_colsize]);
								//interpolate right_patch
								dx          =  pos_col_right - (int)(pos_col_right);
								dy          =  pos_row_right - (int)(pos_row_right);
								...
								if((dx != 0 || dy != 0) && inter_flag == 1)
								{
									right_patch       = (double)(_rightimage[ (int)(pos_col_right)		+ (int)(pos_row_right)*R_colsize])	  *(1-dx)*(1-dy) + 
														(double)(_rightimage[((int)(pos_col_right)+1)	+ (int)(pos_row_right)*R_colsize])	  *(dx)*(1-dy) + 
														(double)(_rightimage[ (int)(pos_col_right)		+ ((int)(pos_row_right)+1)*R_colsize])*(1-dx)*(dy) + 
														(double)(_rightimage[((int)(pos_col_right)+1)	+ ((int)(pos_row_right)+1)*R_colsize])*(dx)*(dy);
								}
								else
									right_patch   =		(double)(_rightimage[ (int)(pos_col_right)		+ (int)(pos_row_right)*R_colsize]);
...
float MergeTiles(ProInfo info,int iter_row_end,int t_col_end, int buffer)
{
	...
	//printf("start merge\n");
	#pragma omp parallel for shared(row_end,col_end,info,find_level,find_iter) private(index_file)
    ...
    #pragma omp parallel for shared(DEM,DEM_size) private(index_file)
    ...
    if(check_gs)
    {
        ...
        #pragma omp parallel for shared(DEMinter,DEMinter_size) private(index_file)
    ...
    //setting DEM value
    #pragma omp parallel for shared(DEM,grid_size,boundary,DEM_size,buffer) private(index_file)
    for(index_file = 0 ; index_file < row_end*col_end ; index_file++)
    {
        int row,col;
        FILE *pfile;
...
CSize Envihdr_reader_seedDEM(char *filename, float *minX, float *maxY, float *grid_size)
{
	...
	while(!feof(fid))
	{
		...
		{
			sscanf(bufstr,"map info = {%s %s %d%s %d%s %f%s %f%s %f%s %f%s %s %s\n",&t_str1,&t_str2,&t_int1,&t_str,&t_int2,&t_str,&(*minX),&t_str,&(*maxY),&t_str,&(*grid_size),&t_str,&(*grid_size),&t_str,&t_str,&t_str);
		}
	}
...
void orthogeneration(char *ImageFilename, char *DEMFilename, char *Outputpath)
{
	...
	SetTransParam_ortho((float)(minLat), &Hemisphere, &param);
	
	//printf("Hemis %d\n",Hemisphere);
	// load DEM infor from hdr file.
	DEM_size = Envihdr_reader_DEM_ortho(DEM_header, &DEM_minX, &DEM_maxY, &DEM_resolution);
	...
	// load DEM value;
	DEM_value = LoadDEM_ortho(DEMFilename, DEM_header);
	...
	for(i=0; i < subfactor;i++)
	{
		for(j=0; j<subfactor;j++)
		{
			...
			if(check_subsetImage)
			{
				...
				int count;
#pragma omp parallel for shared(X_size,Ortho_resolution,Y_size,DEM_minX,DEM_maxY,DEM_resolution,DEM_size,DEM_value,param,startpos,impyramid_step,Image_size,pyimg,OrthoBoundary,Orthoimagesize,result_ortho,col_size,row_size) private(count)
				for(count = 0; count < col_size*row_size ; count++)
				{
						   ...
						   float *subBoundary, float *minmaxHeight, int *cols, int *rows)
{
	...
	if(GetImageSize_ortho(ImageFilename,Imagesize))
	{
		...
		t_pts1			= ps2wgs_3D_ortho(true,transparam,8,t_pts);
							...
							double **RPCs, float gridspace, CSize DEM_size, float minX, float maxY, TransParam param)
{
	...
	XY			= wgs2ps_ortho(false,param,4, LonLat);
...
float *LoadDEM_ortho(char *DEM_path, char* hdr_path)
{
    ...
    float *seeddem = NULL;
	printf("hdr path %s\n",hdr_path);
	seeddem_size  = Envihdr_reader_DEM_ortho(hdr_path, &minX, &maxY, &grid_size);
	maxX	= minX + grid_size*seeddem_size.width;
	minY	= maxY - grid_size*seeddem_size.height;
